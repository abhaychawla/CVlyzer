...
export class AuthenticationService {
  ...
  register(registerContext: RegisterContext) {
    ...
    return this.http.post(`http://localhost:8080/api/user/register`, registerContext)
      .pipe(
        map((response: any) => {
          this.alertService.alert({ type: 'Registration Success', message: `${response.user.username} successfully registered!` });
          return of(true);
        })
      );
  }
  ...
  private onLoginSuccess(credentials: any) {
    ...
    this.alertService.alert({ type: 'Authentication Success', message: `${credentials.user.username} successfully logged in!` });
  }
